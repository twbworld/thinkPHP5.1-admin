...
context = (function () {
        ...
        for (var i = 0; i < data.length; i++) {
            if (typeof data[i].divider !== 'undefined') {
                ...
                if (typeof data[i].action !== 'undefined') {
                    ...
                    $sub
                        .find('a')
                        .addClass('context-event tabReload dropdown-item')
    ...
    function destroyContext(selector) {
        $(document).off('contextmenu', selector).off('click', '.context-event tabReload dropdown-item');
    }
...
var addTabs = function (options) {
    ...
    if (findTabPanel(pageId) === null) {
        ...
        //是否允许关闭
        if (options.close) {
            var $i = $("<i class='fas fa-times fa-lg ml-1 page_tab_close' style='cursor: pointer' onclick='closeTab(this);'></i>").attr(pageIdField, pageId).appendTo($title);
            // title += ' <i class="fas fa-times page_tab_close" style="cursor: pointer;" data-id="' + pageId + '" onclick="closeTab(this)"></i>';
        }
...
function refreshTabById(pageId) {
    ...
    if (url.indexOf(top.document.domain) < 0) {
        $asasd=$iframe.attr("src", url);// 跨域状况下，重新设置url
        if ($asasd) {layer.close(layer.index);}//关闭最新弹出层
...
//菜单栏高亮
    $(".slimScrollDiv").find("a").each(function () {
        var $a = $(this);
            $a.removeClass("menu_active");
        if ($a.attr(pageIdField) == pageId) {
            $a.addClass("menu_active");
        }
    });
        ...
        function init($menu_ul, data, level) {
            $.each(data, function (i, item) {
                ...
                if (item.children && item.children.length > 0) {
                    ...
                    var pullIcon = $('<i class="fas fa-angle-left pull-right"></i>');
